...
function getCity() {
    ...
    console.log(`The coordinates of Evanston are ${evanstonWeather.city.coord.lat} latitude and ${evanstonWeather.city.coord.lon} longitude.`)
    return evanstonWeather.city.name
}
getCity()
for(let timeList of evanstonWeather.list) {
    if(timeList.dt_txt === "2018-03-20 18:00:00")
    {
        console.log(timeList.dt_txt+": There will be "+timeList.weather[0].description+".")
        break
    }
}
for(let timeList of evanstonWeather.list) {
    if(timeList.dt_txt === "2018-03-20 09:00:00")
    {
        console.log(timeList.main.temp)
        console.log(timeList.dt_txt+": It will be "+kelvinToFarenheit(timeList.main.temp)+" Farenheit.")
        break
    }
}
let degrees = 0
let direction  = ""
for(let timeList of evanstonWeather.list) {
    if(timeList.dt_txt === "2018-03-17 15:00:00") {
        console.log("It is currently blowing at "+timeList.wind.speed*2.237+" mph towards the "+getCardinal(timeList.wind.deg))
    }
}
let gross = ""
for(let timeList of evanstonWeather.list){
    if(timeList.main.humidity>75)
        gross = " gross"
    else
        gross = ""
    console.log(timeList.dt_txt+": "+timeList.main.humidity+"%"+gross)
}
function weatherObjConstructor(date, weatherDesc, highTemp, lowTemp){
    let weather = Object.create(null)
    weather.date = date
...
function getWeatherArray(data){
    let returnWeather = []
    for(let dayWeather of data.list) {
        returnWeather.push(weatherObjConstructor(new Date(dayWeather.dt_txt).toDateString(), dayWeather.weather[0].description, Math.floor(kelvinToFarenheit(dayWeather.main.temp_max)), Math.floor(kelvinToFarenheit(dayWeather.main.temp_min))))
    }
    return returnWeather
}
function printForecast(weatherArray){
...
printForecast(getWeatherArray(evanstonWeather))
